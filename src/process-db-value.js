...
const ESCAPE_STRING = {
  TSV(v) { return v.replace(/\\/g, '\\\\').replace(/\t/g, '\\t').replace(/\n/g, '\\n'); },
  CSV(v) { return v.replace(/"/g, '""'); },
};
...
function encodeValue(quote, v, _format) {
...
function encodeRow(row, _format) {
  const format = ALIASES[_format] || _format;

  let encodedRow;

  if (Array.isArray(row)) {
    encodedRow = `${row.map((field) => {
      return encodeValue(false, field, format);
    }).join(SEPARATORS[format])}\n`;
  } else if (row.toString() === '[object Object]' && format === 'JSONEachRow') {
    encodedRow = `${JSON.stringify(Object.keys(row).reduce((encodedRowObject, k) => {
